...
export function Footer({
  ...
  const footerContent = <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-8 md:gap-12 mb-12">
        {/* Company */}
        <div>
          <h4 className="font-semibold text-sm mb-4 text-background/80">{t('footer.company')}</h4>
        ...
        {/* Products */}
        <div>
          <h4 className="font-semibold text-sm mb-4 text-background/80">{t('footer.products')}</h4>
          <ul className="space-y-2.5">
        ...
        <div className="col-span-2 md:col-span-1">
          <h4 className="font-semibold text-sm mb-4 text-background/80">{language === 'ko' ? '패밀리 사이트' : 'Family Site'}</h4>
          <ul className="space-y-2.5">
      ...
      <div className="pt-10 border-t border-background/15">
        <div className="flex flex-col lg:flex-row justify-between gap-8">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-background/60 leading-relaxed max-w-md whitespace-pre-line">
              {language === 'ko' 
          ...
          {/* Contact */}
          <div className="flex-col gap-4 sm:gap-8 text-sm text-background/60 sm:flex-col flex items-start justify-center">
            <a href="mailto:support@i-screamarts.com" className="inline-flex items-center gap-2 hover:text-background transition-colors duration-200 underline-offset-4 hover:underline min-h-[2.75rem] sm:min-h-0">
              <Mail className="w-4 h-4" />
              support@i-screamarts.com
            </a>
            <a href={language === 'ko' ? 'tel:1833-2477' : 'tel:+82-1833-2477'} className="inline-flex items-center gap-2 hover:text-background transition-colors duration-200 underline-offset-4 hover:underline min-h-[2.75rem] sm:min-h-0">
